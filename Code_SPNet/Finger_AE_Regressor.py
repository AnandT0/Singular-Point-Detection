...
def Regressor(input_img, decoded):
	...
	regress = Model(input = [input_img, decoded], output =  fc4)
	#regress.compile(optimizer = Adam(0.0005), loss= 'mean_squared_error')
	return regress
...
# Regressor
#regress = Regressor(input_img, input_img)
reg = Regressor(input_img, input_img)

#Combined
output_img = [input_img, decoded([neck(encoded(input_img)[0]), encoded(input_img)[1], encoded(input_img)[2], encoded(input_img)[3]])]
model= Model(input = input_img, output = output_img)




#model = Model(input = input_img, output = [output_img,reg] )
model.summary()
model.compile(optimizer = Adam(0.0005), loss= ['binary_crossentropy', 'mean_squared_error'], metrics = ["accuracy"])
