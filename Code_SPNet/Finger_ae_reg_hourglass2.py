...
if args.phase=='train':
	...
	test_img=os.path.join(test_input,"data")
...
def load_data():
	...
	y = np.array(gt)
	z = np.array(mask)
	X_train,X_test,Y_train,Y_test, Z_train, Z_test=train_test_split(x,y,z,test_size=0.02)
...
model.compile(optimizer = Adam(0.00005), loss= losses)
...
if args.phase == 'train':
	...
	while epoch <numEpochs :
		...
		history=model.fit(X_train, {'Mask_output':Mask_train,'Output_layer':Y_train}, batch_size=batch_size, epochs=1, validation_data=(X_test,{'Mask_output':Mask_test,'Output_layer':Y_test}), shuffle=True, verbose=1)
	...
	for filename in files:
		...
		f.write(str((y_test[0][0][1]*original_shape1)/y_shape)+" "+str((y_test[0][0][0]*original_shape2)/x_shape))
		f.close()
